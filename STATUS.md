...
- Slice 4b (2026-09-06): Poker now has one ledger, a measured inset MUCK well, canonical numeric CALL/RAISE labels, and a centered YOUR TURN/action dock with no 375px overlap. Public and local 15-action QA pass at 375x812 and 1440x900 with zero browser errors; the 5.95 MB export is live.
- Slice 4c (2026-09-06): Old Maid now uses a dedicated pair-and-draw physical-card table with accessible object controls, honest maid-holder end copy, no stale waiting hint, and measured nav-safe geometry. Public and local QA passed 46 assertions across 375x812 and 1440x900 with zero browser errors; the 5.96 MB export is live.
- Slice 4d (2026-09-06): Klondike now groups STOCK/WASTE and FOUNDATIONS in a single top row, makes the stock itself the draw control, keeps the seven-column tableau inside 375px, and removes the duplicate rule rail. Local and public QA pass 34 assertions at 375x812 and 1440x900 with zero browser errors or viewport overflow; 324/324 Jest, lint, typecheck, Expo Doctor 18/18, and the 5.97 MB export are green and live.
- Slice 4e (2026-09-06): the pass veil is now a quiet felt privacy seal rather than a tutorial overlay. It uses readable PASS TO / recipient / Hold to reveal hierarchy, safe-area padding, and a clearly inset 0.6s hold cue; fresh local and public QA passed 40 checks across 375x812 and 1440x900 with zero overflow or browser errors; 324/324 Jest, Expo Doctor 18/18, the 5.97 MB export, and public HTTP 200 are green.
- Slice 4g (2026-09-07): the store is honest. The flattened Host+Cosmetic bundle card is split into two real Master-benefit tiles (8 players online / 6 offline, every deck), "-24h / -Lifetime" durations are spelled out, decks carry their real names (Deckd Crimson, Noir, Crimson) with state pills (Equipped/Equip) on an inset stage instead of mixed FREE/price semantics, the recipe duplicate ("Choose a recipe", "013", "FLIP A DECK TO PREVIEW") and the redundant Home back pill are cut, and an honest "free in preview" note sits above Restore. Content is a centered capped column at mobile and desktop with wrapped tile/deck grids — no hidden horizontal scroll, no clipped tiles, no left-anchored void. 65-check store QA green local + public at 375x812 and 1440x900, zero overflow/browser errors; 324/324 Jest, lint 0/0, typecheck clean, 5.97 MB export live.
- Slice 5 (2026-09-07): architecture background. Chrome stops re-rendering on every engine event: HomeLayer/HubLayer/TableShell/UtilityDrawer now subscribe narrow primitives or stable refs, `useTableSession` derives from meta/player-key/currentPlayerId instead of the full state, event-log subscriptions moved behind the history modal, and RevK faces render through the memoized `CardFaceArtwork`. The event log is bounded: `compactEventLog` folds old events into the `eventBaseState` baseline and keeps a 500-event tail without breaking replay, undo, persistence, remote fold, or sequence continuity (5 new tests). Lucide imports are per-icon, and 37 QA scripts were deleted in favour of two consolidated suites: `deckd-visual.cjs` (132+ checks across variants at 375x812 + 1440x900, plus the five dedicated game-table library suite and the CardLab 216-face deck proof) and `deckd-lobby-e2e.cjs` (three live two-client phases: Deal-2 sync, online blackjack TWIST, and Hold'em street progression with WebSocket privacy analysis). A web-only RevenueCat stub (Metro resolver) removes the inert web paywall SDK, cutting the entry from 4.13 MB to 3.15 MB raw / 823 KB gzip. Gates: 329/329 Jest, lint, typecheck, QA green, export deployed.
- Owed proof prep (2026-09-07): live 2-device run prepared, not yet executed. Fresh build deployed (entry-72cfe27a, HTTP 200, 3.31MB raw entry) and a step-by-step human test script saved at `qa/two-device-proof.md` (3 phases: Deal-2 lobby sync, online blackjack guest TWIST, poker streets + privacy check, with pass criteria and failure protocol). The automated two-client E2E (`qa/deckd-lobby-e2e.cjs`) passes against the same build; the real-device run needs Josh's hands.
- Owed P0 (2026-09-07): hold-to-peek dual-end mode shipped. `DualEndTable` puts both seats on one phone from opposite ends (Player B's strip rotated 180, shared deck/discard centre, PASS TURN on the centre line, no device-handing veil); hold your end to reveal, release to re-veil, and a second finger tap on a revealed card discards it. Rescoped per CLEANUP §4.1: no new engine events — both hands render from canonical state, and `card/peek` stays a networked projection hook. Eligible for 2-player freeplay and blackjack (virtual house seat excluded from the two-seat check). Gates: 335/335 Jest (6 dual-end tests), lint 0/0, typecheck clean, QA green at 375x812 + 1440x900, export live.
