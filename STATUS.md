...
- Cold-load card readiness: the root layout preloads the canonical logo and all three branded backs before hiding the splash, so a fresh Home frame does not show blank paper shells while RN Web image assets arrive.
...
- Release: `npm run check`, `npm run test` (62/62), static export, PM2 `deckd-app` restart, public HTTP 200, and a deployed 375×812 flow through `PASS TURN` all pass with zero browser console/page errors.
